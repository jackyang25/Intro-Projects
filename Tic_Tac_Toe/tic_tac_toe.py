...
def display_outcome(round):
    ...
    elif mod == 0 and round <= MAX_ROUNDS:
        print("X wins!")
        print()

    elif mod == 1 and round <= MAX_ROUNDS:
        print("O wins!")
        print()
...
def check_positions(pos1_value, pos2_value, pos3_value):
    ...
    False
    """
    if pos1_value == " " and pos2_value == " " and pos3_value == " ":
        return False

    elif pos1_value == pos2_value == pos3_value:
        return True
...
def is_game_complete(board):
    ...
    elif check_positions(board[2, 0], board[1, 1], board[0, 2]) is True:
        return True
...
def play_tic_tac_toe(board):
    ...
    while (is_game_complete(board) == False) and (round < MAX_ROUNDS):
    ...
    finished = is_game_complete(board)
    if finished:
        display_outcome(round - 1)
    elif round == MAX_ROUNDS:
        display_outcome(round)

    play_again = is_program_finished()
